...
const double       tHighThresh = 95.0;      //SET MAX TEMP IN THE CYCLE
const double       rampUpThresh = -34.0;     //SET MIN UP 'on' IN THE CYCLE
const double       tLowThresh = -40;       //SET MIN TEMP IN THE CYCLE

const double       rampDownThresh = 80.0;   //SET MIN DOWN 'off' in THE CYCLE
...
//////////////////////////////////RAMP UP ///////////////////////////////////////////// 
 ...
 if ( (TCtemp>tLowThresh) and (TCtemp>=rampUpThresh) and ( TCtemp<(tHighThresh*1.05))and(tSlope>=0) ) {
...
//////////////////////////////////RAMP DOWN ///////////////////////////////////////////// 
 else if ( (TCtemp>tLowThresh) and (TCtemp>rampDownThresh) and ( TCtemp<(tHighThresh*1.05))and (tSlope<0) ){
 ...
 /////////////////////////////////FINAL EXECUTION////////////////STARTS WITH LEAVE ON***/////////////////////////////////////    
if( ((val < alarmThresh) and  ( ((TCtemp>tLowThresh) and (TCtemp>rampUpThresh) and (TCtemp<(tHighThresh*1.05)))and (tSlope>=0) )  ) or
    
    ((val > alarmThresh) and  ( ((TCtemp>tLowThresh) and (TCtemp>rampDownThresh) and ( TCtemp<(tHighThresh*1.05)))and((tSlope<0) ))  )
  ){
      for (int i=0; i<4;  i++){
              if(relayUse[i]==1) {
